...
class (Ord a) => PQueue pq a where
	...
	toList ppq
		| isEmpty ppq = []
		| otherwise = (fromJust (top ppq)) : (toList (pop ppq)) 

    size :: pq a -> Int;
	size pqa 
		| isEmpty pqa = 0
		| otherwise = 1 + (size (pop pqa))
	-- Pentru Ex 6
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert x (LPQ []) = (LPQ [x])
    insert x (LPQ l) = LPQ (sortBy f (x : l))
...
merge a b
	| a == (Empty _) && b == (Empty _) = (Empty 0)
	| a == (Empty _) = b
	| b == (Empty _) = a
	| otherwise = if (fst (nodeVal a)) >= (fst (nodeVal b))
					then
...
inorder (Empty _) = []
inorder l = (inorder (left l)) ++ (nodeVal l) ++ (inorder (right l))
