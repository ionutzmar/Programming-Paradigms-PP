...
orthogonalV vec = if suma == 0 then True else False
...
insertElem (Nod n left right) x
	| x == n = Nod n left right
	| x < n = Nod n (insertElem left x) right 
	| otherwise = Nod n left (insertElem right x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (Nod n left right) x
	| x == n = Just x
	| x < n = findElem left x
...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder (Nod a left right) = inorder left ++ [a] ++ inorder right
