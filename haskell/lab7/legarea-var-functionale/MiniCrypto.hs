...
arithmetic a r = iterate (+ r) a
geometric :: Num a => a -> a -> [a]
geometric b r = iterate (* r) b
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = map (+ 1) (randomRs (0, 255) $ mkStdGen seed)
{-
    4. (1p)
...
wordToAlpha w = chr $ ((mod (fromIntegral w) 26) + (ord 'a'))
{-
    5. (1p)
...
tableToFunc table  = (\a -> snd ( head (filter (\x -> a == (fst x)) table)))
