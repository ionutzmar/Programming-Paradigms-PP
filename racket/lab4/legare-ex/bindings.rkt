...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let*
      (
       (segment (get-line-segment))
       (x (get-start-point segment))
      (y (get-end-point segment))
      (a (car x))
      (b (cdr x))
      (c (car y))
      (d (cdr y))
...
(define (compute-f-with-step f a b step)
  (let nume((currentStep a)
            (rez null))
    (if (> currentStep b)
        rez
        (nume (+ currentStep step) (append rez (list (f currentStep)))))
    ))
...
(define (compute-perimeter points)  
  (let nume((p points)
            (parameter 0))
    (if (null? (cdr p))
        (+ parameter (distance (car points) (car p)))
        (nume (cdr p) (+ parameter (distance (car p) (cadr p)))
        )
    )))
...
(define (3-sequence-max numbers separator)
  (let nume ((max 0)(L numbers))
    (let-values ([(low high)(splitf-at L (lambda (x) (if(= x separator)#f #t)))])
      (if (null? low)
          max
          (if (> (summ low) max)
              (nume (summ low)(rest high))
              (nume max (rest high)))))))
