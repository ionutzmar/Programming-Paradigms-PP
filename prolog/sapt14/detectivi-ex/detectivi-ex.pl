...
arme_bmw(ListaArme) :- findall(X, (conduce(Nume, bmw), inarmat(Nume, X)), ListaArme).
...
arme_marca(Marca, ListaArme) :- findall(X, (conduce(Nume, Marca), inarmat(Nume, X)), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, (conduce(Nume, Marca), inarmat(Nume, Arma)),ListaMarci).
