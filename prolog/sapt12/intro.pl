...
myConcat([],L,L).
myConcat([Head|Tail],L,[Head|Tail2]):- myConcat(Tail, L, Tail2).
...
myReverse([],[]).
myReverse([Head|Tail], RevList):- myReverse(Tail, Aux), myConcat(Aux,[Head], RevList).
...
myReverseAcc([],L,L).
myReverseAcc([Head|Tail], Acc, L):-myReverseAcc(Tail, [Head|Acc], L).
...
extract(L1, Start, End, L2):- extractHelper(L1, 0, Start, End, L2).
extractHelper([H|_], End, _, End, [H]).
extractHelper([],_,_,_,[]).
extractHelper([H | T], I, Start, End, [H | L]) :- I >= Start, I1 is I + 1, extractHelper(T, I1, Start, End, L).
extractHelper([_ | T], I, Start, End, L) :- I1 is I + 1, extractHelper(T, I1, Start, End, L).
...
factorial(0,1).
factorial(N, Fact):-  N1 is N - 1, factorial(N1, F), Fact is  F * N.
...
factorial2(0,1).
factorial2(N, Fact):- factorial2(N1, F1), N is N1 + 1, Fact is F1 * N.
